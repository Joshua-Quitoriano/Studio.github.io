...
try {
    ...
    $check_table = $conn->query("SHOW TABLES LIKE 'college_courses'");
    ...
    if ($check_table->num_rows === 0) {
        ...
        $create_table = "CREATE TABLE IF NOT EXISTS college_courses (
        ...
        if (!$conn->query($create_table)) {
            throw new Exception("Failed to create college_courses table: " . $conn->error);
        }
        error_log("Created college_courses table");
    }

    // Check if there are any courses
    $count = $conn->query("SELECT COUNT(*) as total FROM college_courses");
    if ($count === false) {
    ...
    if ($count_result['total'] == 0) {
        ...
        $conn->query("TRUNCATE TABLE college_courses");
        error_log("Truncated college_courses table");

        // Now insert the courses
        foreach ($default_courses as $course) {
            $sql = "INSERT INTO college_courses (code, name) VALUES ('" . 
    ...
    $result = $conn->query("SELECT id, code, name FROM college_courses ORDER BY name");
    if ($result === false) {
